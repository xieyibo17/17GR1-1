...
<script>
function jiarugouwuche(id){
	alert(id);
	$.getJSON("servlet/ProductServlet","pro=cxwpxx&id="+id,function(json){
		var num=$(".car_t span").val();
		if(num=="空"){
			alert(num);
			num=0;
		}
		$(".car_t span").html(num+1);
		$(".cars").append("<li>"+
				"<div class='img'>"+
					"<a href='#'><img src='images/"+json.fileName+".jpg' width='58' height='58' /></a>"+
				"</div>"+
				"<div class='name'>"+
					"<a href='#'>"+json.description+"</a>"+
				"</div>"+
				"<div class='price'>"+
					"<font color='#ff4e00'>￥"+json.price+"</font> X1"+
				"</div>"+
					+"</li>");
	});	
...
function huoqushangpin(id) {
	$.getJSON("servlet/ProductServlet","pro=zssp&sjid="+id,function(json){
	var $jjj=$(json);
	$(".menu_bg").empty();
	$(".cate_list").empty();
	$jjj.each(function(i,v){
		$(".cate_list").append("<li>"
            	+"<div class='img'><a href='#'><img src='images/"+v.fileName+"' width='210' height='185' /></a></div>"
            	+"<div class='price'>"
            	...
            	+"   <a href='#' onclick='jiarugouwuche("+v.id+")' class='j_car'>加入购物车</a>"
            	+" </div>"
            +"</li>");
	});
	});
	...
	<div class="soubg">
		<div class="sou">
			...
			<span class="fr"> <c:if test="${not empty sessionScope.name}">
					<span class="fl"><a href="#">${sessionScope.name}</a>&nbsp;|</span>
		...
		<div class="menu">
			...
			<div class="nav">
				...
				<div class="leftNav">
                   ...
                   int i=1;
                   for(ProductCategoryVo yiji:queryAllProductCategoryList){
                	   %>
						<li><div class="fj">
								<span class="n_img"><img src="images/nav<%=i%>.png"/></span> 
								<span
									class="fl"> <%=yiji.getProduct_category().getName()%>
								</span>
							</div>
							<div class="zj">
								<div class="zj_l">
									<%
                	   i++;
                	  List<ProductCategoryVo> erjishuzhu =yiji.getProductDaXiangzi();
                	   for(ProductCategoryVo erji:erjishuzhu){
                		   %>
									<div class="zj_l_c">
										<h2><%=erji.getProduct_category().getName()%></h2>
										<%
                		   List<ProductCategoryVo> sanjishuzhu=erji.getProductDaXiangzi();
                		   for(ProductCategoryVo sanji:sanjishuzhu){
                			   %>
										<a href="#"
											onclick="huoqushangpin(<%=sanji.getProduct_category().getId()%>)"><%=sanji.getProduct_category().getName()%></a>
										<%
                		   }
                		   %>
									</div>
	...
	<ul class="cate_list">
        ...
        (function(){
			...
			<div class="inews">
				...
				<ul>
					<c:if test="${empty sessionScope.newslist}">
					<script>
					location.href="servlet/NewsServlet?&&pro=allnews";
					</script>
					</c:if>
					<c:if test="${not empty sessionScope.newslist}">
					<c:forEach items="${sessionScope.newslist}" var="news">
					<li style="width:205px;	height:30px"><span>[ 公告 ]</span><a href="#">${news.title}</a></li>
					</c:forEach>
					</c:if>
				</ul>
				<div class="charge_t">
